...
-(id)initWithParentView:(UIScrollView *)parentView {
...
-(void)show:(CGPoint)scrollContentOffset {
    //self.hidden = NO;
    ...
    if (_parentView != nil) {
        ...
        if (_isInitialized == NO) {
            self.frame = CGRectMake(self.frame.origin.x, _parentView.contentOffset.y+(135*2), self.frame.size.width, self.frame.size.height);
            _isInitialized = YES;
        }
...
-(void)dismiss {
    ...
    if (_parentView != nil) {
        ...
        [UIView beginAnimations:@"FlatDatePickerShow" context:nil];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDuration:0.5];//kFlatDatePickerAnimationDuration];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDidStopSelector:@selector(animationDismissDidFinish)];
        
        self.frame = CGRectMake(self.frame.origin.x, _parentView.frame.size.height+(135*2), self.frame.size.width, self.frame.size.height);
        //NSLog(@"SELF HEIGHT: %f", _parentView.frame.size.height);
        self.alpha = 0.5;
